(function (e) { typeof define === 'function' && define.amd ? define(['jquery'], e) : typeof exports !== 'undefined' ? module.exports = e(require('jquery')) : e(jQuery); }((e) => {
  ...
  } function X(a) { return (a = a.data('scrollax')) && eval(`({${a}})`) || {}; } function Y(a) { let g; let c; return !!(a && typeof a === 'object' && typeof a.window === 'object' && a.window == a && a.setTimeout && a.alert && (g = a.document) && typeof g === 'object' && (c = g.defaultView || g.parentWindow) && typeof c === 'object' && c == a); } var v = Array.prototype; const C = v.push; const Z = v.splice; const aa = Object.prototype.hasOwnProperty; const la = /[-+]?\d+(\.\d+)?/g; const ma = 'translateX translateY rotate rotateX rotateY rotateZ skewX skewY scaleX scaleY'.split(' ');
  const ba = e(window); const ca = e(document.body); let da; let ea; let L; let M; let N; const q = function (a, g, c) {
    function k() { O = fa ? ca.find(ga) : P.find(ga); x.length = 0; r = !!t.horizontal; O.each(na); d(); t.performanceTrick && (F = fa ? ca : P); u('load'); return f; } function l() { G && (G = clearTimeout(G)); G = setTimeout(() => { f.reload(); }); } function d() {
      const ha = x.length; t.performanceTrick && F && (clearTimeout(ia), Q || (F.addClass('scrollax-performance'), Q = !0), ia = setTimeout(() => { F.removeClass('scrollax-performance'); Q = !1; }, 100)); if (ha) {
...
*= d;
    ...
    } function p(a, b) { for (let c = 0, f = h[a].length; c < f; c++) if (h[a][c] === b) return c; return -1; } var f = this; var P = a && e(a).eq(0) || ba; const w = q.instances; let v = null; a = P[0]; e.each(w, (b, c) => { b && b.frame === a && (v = !0); }); if (!a || v)v ? W('Scrollax: Scrollax has been initialized for this frame!') : W('Scrollax: Frame is not available!'); else {
      ...
      }; f.off = function (a, c) { if (c instanceof Array) for (var b = 0, d = c.length; b < d; b++)f.off(a, c[b]); else for (var b = a.split(' '), d = 0, g = b.length; d < g; d++) if (h[b[d]] = h[b[d]] || [], typeof c === 'undefined')h[b[d]].length = 0; else { const k = p(b[d], c); k !== -1 && Z.call(h[b[d]], k, 1); } return f; }; f.set = function (a, b) {
        e.isPlainObject(a) ? e.extend(t, a) : aa.call(t, a) && (t[a] = b); k();
        return f;
      }; f.destroy = function () { N(window, 'resize', l); N(a, 'scroll', d); e.each(w, (b, c) => { b && b.frame === a && Z.call(q.instances, c, 1); }); x.length = 0; f.initialized = !1; u('destroy'); return f; }; f.init = function () { if (!f.initialized) return f.on(c), k(), M(window, 'resize', l), M(a, 'scroll', d), C.call(q.instances, f), f.initialized = !0, u('initialized'), f; };
    }
  }; q.instances = []; (function () {
    let a; let g; let c; let k; let l; let d; let b; let e; L = function (u, p) {
...
+ a.charAt(0).toUpperCase() + a.slice(1) : a; if (c.style[b] != null) return b;
    ...
    horizontal: !1, offset: 0, parentSelector: null, elementsSelector: null, performanceTrick: !1,
  }; window.Scrollax = q; e.fn.Scrollax = function (a, g) {
    let c; let k; if (!e.isPlainObject(a)) { if (typeof a === 'string' || !1 === a)c = !1 === a ? 'destroy' : a, k = slice.call(arguments, 1); a = {}; } return this.each((l, d) => {
      const b = e.data(
        d,
        'scrollax',
